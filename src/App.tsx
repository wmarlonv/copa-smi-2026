...
// ============================================================================
// 2. COMPONENTES ROCKETSEAT STYLE
// ============================================================================
...
const Button = ({ onClick, children, variant = 'primary', className = "", type = "button" }) => {
  ...
  return <button type={type} onClick={onClick} className={`px-4 py-2 rounded-md font-bold text-sm transition-all flex items-center justify-center gap-2 ${styles[variant]} ${className}`}>{children}</button>;
};

const Badge = ({ children, color = "gray" }) => {
  const colors = { 
    red: "bg-red-500/10 text-red-500", 
    gray: "bg-[#29292E] text-[#8D8D99]", 
    green: "bg-[#04D361]/10 text-[#04D361]", 
...
export default function TournamentApp() {
  ...
  const handleLogin = (e) => {
    ...
    if (pwdInput && pwdInput.value === ADMIN_PASSWORD) {
        setIsAdmin(true);
        setActiveTab('dashboard'); // Joga pro início ao logar
        pwdInput.value = '';
    } else {
  ...
  const handleLogout = () => {
    setIsAdmin(false);
    if (activeTab === 'financial_hub') {
        setActiveTab('dashboard');
    }
  };

  const savePlayerEdit = (pid) => {
    setPlayers(players.map(p => p.id === pid ? { ...p, ...editForm, manualPts: Number(editForm.manualPts) || 0 } : p));
    setEditingPlayerId(null);
  };

  const handleAddPlayer = () => { if(newPlayerName) { setPlayers([...players, {id:Date.now(), name:newPlayerName, side:newPlayerSide, uniformPaid:false, manualPts: 0}]); setShowAddPlayerForm(false); setNewPlayerName(""); }};
  ...
  const redoDraw = (sid) => {
    if(window.confirm("Atenção! Isso apagará todos os grupos e jogos desta etapa. Deseja refazer o sorteio?")) {
        updateStage(sid, { status: 'registration', pairs: [], groups: [], matches: [], tv: { q1: null, q2: null } });
    }
  };
  ...
  if (isAdmin) {
    menuItems.push({ id: 'financial_hub', icon: DollarSign, label: 'Financeiro' });
  }
      ...
      return (
        <div className="min-h-screen bg-[#121214] flex items-center justify-center p-4">
          <div className="bg-[#202024] w-full max-w-md rounded-md overflow-hidden shadow-2xl border border-[#323238] relative">
            ...
            <div className="bg-[#121214] p-4 flex justify-between items-center border-t border-[#323238]"><div className="flex gap-3 grayscale opacity-50">{BRAND.sponsors.slice(0,3).map((s,i)=><img key={i} src={s} alt="Sponsor" className="h-4 object-contain" />)}</div><img src={BRAND.arenaLogo} alt="Arena" className="h-6 grayscale opacity-50"/></div>
          </div>
  ...
  if(tvMode) {
      ...
      return (
          <div className="h-screen bg-[#09090A] text-[#E1E1E6] flex flex-col p-6 gap-6 font-sans">
              <div className="flex justify-between items-center pb-4 border-b border-[#323238]"><img src={BRAND.logo} alt="Logo" className="h-12"/><div className="text-right"><div className="text-xs text-[#04D361] font-bold uppercase tracking-widest flex items-center justify-end gap-2"><span className="w-2 h-2 rounded-full bg-[#04D361] animate-pulse"></span> Ao Vivo</div><div className="text-xl font-black uppercase tracking-tight">{stage?.name}</div></div><button onClick={()=>setTvMode(false)} className="opacity-0 hover:opacity-100">X</button></div>
                      ...
                      return (
                          <Card key={idx} className="h-full flex flex-col justify-center items-center p-6 relative overflow-hidden bg-[#202024]">
                               ...
                               <div className="flex w-full h-full justify-between items-center gap-4">
                                   <div className="flex-1 text-center space-y-2"><div className="text-4xl font-black text-[#E1E1E6] truncate uppercase">{players.find(p=>p.id===m.pa.p1)?.name}</div><div className="text-2xl font-bold text-[#8D8D99] truncate uppercase">{players.find(p=>p.id===m.pa.p2)?.name}</div></div>
                                   <div className="px-6 text-8xl font-black text-yellow-500 tabular-nums">{m.sA}<span className="text-[#323238] text-6xl mx-4">:</span>{m.sB}</div>
                                   <div className="flex-1 text-center space-y-2"><div className="text-4xl font-black text-[#E1E1E6] truncate uppercase">{players.find(p=>p.id===m.pb.p1)?.name}</div><div className="text-2xl font-bold text-[#8D8D99] truncate uppercase">{players.find(p=>p.id===m.pb.p2)?.name}</div></div>
                               </div>
                          </Card>
                      )
                  })}
              ...
              <div className="h-16 bg-[#202024] rounded-md flex items-center justify-between px-8 border border-[#323238]"><div className="flex gap-6 grayscale opacity-60">{BRAND.sponsors.map((s,i)=><img key={i} src={s} alt="Sponsor" className="h-6 object-contain"/>)}</div><img src={BRAND.arenaLogo} alt="Arena" className="h-8 grayscale opacity-50" /></div>
          </div>
      );
  }

  // --- ESTRUTURA DASHBOARD HÍBRIDO (PC & MOBILE) ---
  return (
    <div className="min-h-screen bg-[#121214] font-sans text-[#E1E1E6] flex">
      
      {/* 1. SIDEBAR CONFIG (APENAS PARA COMPUTADOR) */}
      ...
      {/* 2. ÁREA CENTRAL DE CONTEÚDO (COM MARGEM ADAPTADA NO PC) */}
      <div className="flex-1 flex flex-col md:ml-64 min-w-0">
         <header className="bg-[#121214]/90 backdrop-blur-md border-b border-[#323238] p-5 sticky top-0 z-20 flex justify-between items-center max-w-5xl w-full mx-auto md:px-8">
            ...
            <div className="hidden md:flex flex-col">
               <h2 className="text-lg font-bold text-[#E1E1E6]">{currentTabLabel}</h2>
            </div>
            ...
            {activeTab === 'admin_login' && (
               <div className="flex flex-col items-center justify-center min-h-[50vh] animate-in fade-in duration-200">
                  <div className="bg-[#202024] border border-[#323238] p-8 rounded-md shadow-lg w-full max-w-sm text-center">
                     ...
                     {isAdmin && <div className="mt-8 pt-6 border-t border-[#323238]"><button onClick={()=>{if(window.confirm('CUIDADO EXTREMO! Isso limpará TODOS OS DADOS do sistema. Continuar?')){localStorage.clear();window.location.reload()}}} className="text-[10px] font-bold text-red-500 uppercase hover:underline">Zerar Banco de Dados</button></div>}
                  </div>
               </div>
            )}
         </main>
         ...
         <footer className="mt-auto p-6 md:px-8 border-t border-[#323238] flex flex-col md:flex-row items-center justify-center md:justify-between gap-6 opacity-60 hover:opacity-100 transition-opacity w-full max-w-5xl mx-auto pb-28 md:pb-6">
            <div className="flex justify-center md:justify-start items-center gap-4 flex-wrap grayscale brightness-0 invert">
               {BRAND.sponsors.map((s, idx) => (<img key={idx} src={s} alt="patrocinio" className="h-5 md:h-6 object-contain" />))}
            </div>
      ...
      <div className="fixed bottom-0 inset-x-0 z-40 md:hidden bg-[#202024] border-t border-[#323238] pb-safe shadow-2xl">
         <nav className="flex justify-around items-center p-3">
            {menuItems.map(item => { const Icon = item.icon; const isSel = activeTab === item.id; return (
               <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center gap-1.5 transition-colors ${isSel ? 'text-red-500' : 'text-[#8D8D99] hover:text-[#C4C4CC]'}`}>
